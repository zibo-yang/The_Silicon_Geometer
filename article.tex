...
\end{quote}
We should note that the Isabelle proof assistant is based on a much more minimalist type theory, known as a simple type theory. As a consequence, many users of dependent type theories see Isabelle's type theory has a far less expressive system for formalizing advanced mathematics.  These doubts have been vocally expressed by Kevin Buzzard on his blog:
\begin{quote}
...
\end{quote}
In this article we present a concise and elegant formalization of schemes in Isabelle/HOL following the classic graduate textbook of Hartshorne \cite{hartshorne}. 			
...
% snippet of code to be inserted here for the corresponding lemma in Isabelle
...
As sanity checks and to catch on the state of the art in the Lean theorem prover, we prove in Isabelle that an affine scheme is a scheme and we give the example of the empty scheme $(\emptyset, \mathscr{O}_\emptyset)$, where $\mathscr{O}_\emptyset(\emptyset)$ is the zero ring $\lbrace 0 \rbrace$. 
