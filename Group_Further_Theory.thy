...
shows "monoid_homomorphism (\<theta> \<circ> \<eta>) A multA oneA C multC oneC"
proof-
  have "(\<theta> \<circ> \<eta>) oneA = oneC" 
    using assms monoid_homomorphism.commutes_with_unit by (metis comp_eq_elim comp_id id_apply)
  moreover have "\<And>x y. x \<in> A \<Longrightarrow> y \<in> A \<Longrightarrow> (\<theta> \<circ> \<eta>) (multA x y) = multC ((\<theta> \<circ> \<eta>) x) ((\<theta> \<circ> \<eta>) y)" 
    using assms monoid_homomorphism.commutes_with_composition
    by (smt fcomp_apply fcomp_comp map.map_closed monoid_homomorphism_def)
...
shows "group_homomorphism (\<theta> \<circ> \<eta>) A multA oneA C multC oneC"
